...
**Combine 10 & 11 and remove procedure[Fixed!], change the heading**
...
- Trace Plots:
  - Chains are stationary (no drift) 
  - Continuous parameters exhibit “fuzzy caterpillar” behavior → good mixing 
  - $\tau$ concentrated near 30–31 → strong identifiability 

- ACF:
  - $\sigma$ and $\tau$: fast decay → low autocorrelation 
  - $K$, $r_1$, $r_2$, and $t_0$ → persistent dependence
...
**remove slide 15 and just keep the plots** - In slide 15 should we keep acceptance rate and ESS part, only remove trace and ACF??
...
- **ACF:**
  - $\sigma$: fast decay  
  - $\beta_1, \beta_2, \gamma$: slow decay reflecting dependence due to shared ODE structure
...
**Read 31,32,33 and change it accordingly**

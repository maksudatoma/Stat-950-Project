...
\textbf{merge slides 8 and 9}{[}Fixed{]}
\end{frame}
...
\textbf{Combine 10 \& 11 and remove procedure{[}Fixed!{]}, change the
heading}
\end{frame}
...
\item
  ...
  \item
    Continuous parameters exhibit ``fuzzy caterpillar'' behavior → good
    mixing
  \item
...
keep acceptance rate and ESS part, only remove trace and ACF??
\end{frame}
...
\phantomsection\label{sir-model-1}
\begin{itemize}
\tightlist
\item
...
\begin{frame}[fragile]{SIR Change-Point Model}
...
** Combine 20 and 21** \#\# Frequentist Estimation: SIR Model
...
\textbf{Read 31,32,33 and change it accordingly}
\end{frame}

...
**Assumptions:** The model assumes that the total population $N$ is fixed, the initial conditions $S(0)$, $I(0)$, and $R(0)$ are known, and a single change-point occurs in the transmission rate.
...
In contrast, the SIR model shows more nuanced differences.
While parameter recovery remains accurate, bootstrap and Bayesian intervals are broadly similar but not identical.
In particular, Bayesian intervals tend to be slightly wider for some parameters, reflecting their ability to capture posterior dependence and additional uncertainty.

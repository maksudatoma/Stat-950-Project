...
(@fig-hist; @fig-box; @fig-log; @fig-lik)
...
**Table 4: Effective sample sizes (ESS) for each parameter after burn-in and thinning. [Instead of table we can just describe it, right? - I think table looks more clear(just a idea)**
...
**The effective sample sizes after burn-in and thinning were 46 for $K$, 107 for $r_1$, 49 for $r_2$, 54 for $t_0$, 2594 for $\sigma$, and 387 for $\tau$.**
...
**this part does not appear in my pdf (may be a formatting issue)**
...
In contrast, the SIR model shows more nuanced differences. While parameter recovery remains accurate, bootstrap and Bayesian intervals are broadly similar but not identical. In particular, Bayesian intervals tend to be slightly wider for some parameters, reflecting their ability to capture posterior dependence and additional uncertainty. This highlights the importance of accounting for model-induced parameter correlations in mechanistic models.
...
**At very last might put Dixon's paper in appropriate place [Actually neither paper is related to our work. should we need any more reference? - I think this is enough**
...
![Estimated growth rates across simulations.](box.png){#fig-box}
:::

::: {layout-ncol="2"}
![Fitted change-point logistic model compared to the true mean function.](logistic.jpeg){#fig-log}

![Profile likelihood for the change-point parameter $\tau$.](pro_lik.jpeg){#fig-lik}
:::

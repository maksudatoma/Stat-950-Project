...
The results show that the change-point SIR model performs well in both estimation and inference.
...
Across models, the change-point in the SIR model is estimated much more precisely than in the logistic model.
The posterior for $\tau$ collapses to a single value, indicating near-perfect identifiability.
This reflects the mechanistic structure of the SIR model, where a change in transmission rate affects the entire system trajectory, making the timing of the change-point much easier to detect.
...
This project demonstrates that the same inferential pipeline, profile likelihood, parametric bootstrap, and Bayesian MCMC, carries fundamentally different computational implications depending on model structure.
Both models successfully recovered true parameters with negligible bias, and the change-point was accurately identified under all inferential approaches, confirming that the proposed methods are statistically reliable in both the logistic and SIR settings.

The most significant finding is the contrast in change-point identifiability between the two models.
In the logistic model, the posterior for $\tau$ spread across two adjacent values, reflecting moderate identifiability driven by local curvature changes.
In the SIR model, the posterior collapsed to a point mass at the true value, with log-likelihood differences exceeding 50 units between adjacent candidates.
This stark difference arises because a transmission rate change in the SIR model propagates through the entire coupled ODE system, making its timing far more precisely detectable than a growth rate change in the logistic model.

Computationally, the SIR model was substantially more demanding.
The profile likelihood search was 50--100 times slower, the parametric bootstrap required approximately three hours compared to 30 seconds for the logistic model, and MCMC required ODE solving at every iteration.
Despite this cost, MCMC proved more computationally efficient than the bootstrap for the SIR setting, since it amortizes ODE-solving cost across iterations rather than requiring full independent refits.

Both MCMC samplers exhibited low effective sample sizes for structural parameters due to posterior correlation induced by the shared nonlinear mean function.
While thinning and longer chains partially addressed this, gradient-based samplers such as Hamiltonian Monte Carlo would more fundamentally resolve it and represent a natural direction for future work.

...
Bootstrap and Bayesian credible intervals agree closely for the logistic model but diverge in the SIR model, where Bayesian intervals better capture posterior correlation among epidemic parameters.
Computationally, the SIR model is substantially more expensive, requiring ODE solving at every likelihood evaluation and making the profile search approximately 50--100 times slower than the logistic equivalent.
...
![*Estimated growth rates across simulations.*](box.jpeg)
:::
...
The fitted model closely follows the true underlying curve across the full time range.
The estimated change-point aligns closely with the true value, and the model successfully captures the shift in growth rate.
Before the change-point, the curve exhibits a steeper slope corresponding to a higher growth rate.
After the change-point, the slope decreases, indicating slower growth.
This transition is well captured by the model.

The profile likelihood plot shows the negative log-likelihood evaluated across candidate values of the change-point parameter $\tau$.
The minimum of the curve corresponds to the estimated change-point, indicating the value of $\tau$ that best fits the data.
The estimated $\tau$ is very close to the true value, demonstrating accurate detection of the change-point.
The sharp curvature around the minimum suggests that the likelihood is sensitive to changes in $\tau$, indicating good identifiability and relatively low uncertainty in the estimate.
...
Trace plots (Figure 5) show stationarity with no drift, indicating convergence.
...
Figure 6 shows the ACF plots after thinning.
...
The posterior for $\sigma$ is the most precise, nearly symmetric and centered very close to the true value (3.0).
The posteriors for $r_1$ and $K$ exhibit slight right-skew, while $r_2$ and $t_0$ show mild left-skew, reflecting asymmetry in the likelihood.
All true parameter values lie within the 95% credible intervals, indicating good coverage.
The posterior for $\tau$ is sharply concentrated, with about 70% of its mass at 30 and most of the remainder at 31, confirming precise identification of the change-point.
...
The bootstrap confidence intervals and Bayesian credible intervals are in close agreement for all continuous parameters, with similar widths and substantial overlap, as expected under a highly informative likelihood.
The main difference is for the change-point $\tau$: the bootstrap interval $(28,31)$ and Bayesian interval $(30,31)$ are both narrow and contain the true value, but differ slightly because $\tau$ is discrete and sensitive to data variability.
The parameter $K$ has the widest intervals under both approaches, reflecting greater difficulty in estimating the logistic plateau, while $r_1$ is estimated more precisely than $r_2$ because more data are available before the change-point.
Overall, the close agreement between the two methods suggests that the inferential conclusions are robust.
...
Bootstrap datasets were generated from the fitted SIR model by simulating from the estimated mean trajectory and adding Gaussian noise using the estimated error standard deviation.
The change-point model was then refitted to each bootstrap dataset.
This process was repeated for 200 bootstrap replications.
...
The bootstrap distribution of $\tau$ is tightly centered around the true value, indicating stable estimation. All confidence intervals contain the true parameter values. The interval for $\tau$ is relatively narrow, indicating precise estimation of the change-point.

These results show that the change-point SIR model performs well in both estimation and inference.
All parameters are recovered with low bias and variability, and the change-point $\tau$ is estimated accurately and consistently.
The bootstrap procedure provides reliable confidence intervals, while the slightly higher variability in $\beta_2$ reflects the greater difficulty of estimating post-change dynamics.
Overall, the estimation procedure is both numerically stable and statistically reliable under the simulation setting.
...
Figure 11 shows that the posterior fit closely matches both the observed data and the true trajectory, indicating a good model fit. The change-point is correctly identified around  $\tau = 20$, capturing the peak and subsequent decline of the epidemic.
...
For $\beta_1$, both approaches produce narrow, overlapping intervals, indicating consistent estimation.
For $\beta_2$, the Bayesian credible interval $(0.1495, 0.1908)$ is much wider than the bootstrap interval $(0.1669, 0.1758)$, suggesting that the bootstrap may underestimate uncertainty in the post-change transmission rate.
For $\gamma$, the pattern is reversed: the bootstrap interval $(0.1071, 0.1080)$ is much narrower than the Bayesian interval $(0.0996, 0.1106)$, suggesting overconfidence in the bootstrap estimate for this dataset.
...
Runtime comparisons show a clear increase in computational cost from the logistic model to the SIR model.
For the logistic model, the MLE profile search is essentially instantaneous, the parametric bootstrap takes about 30 seconds, and MCMC takes about 3 minutes.
In contrast, the SIR model is much more expensive: the profile search takes 52 seconds, the bootstrap requires about 3 hours, and MCMC takes about 205 seconds.

The largest difference appears in the MLE profile search.
The logistic model is fast because its mean function is available in closed form, whereas the SIR model requires numerical solution of the ODE system at each likelihood evaluation, leading to roughly a 50--100 fold increase in cost.
The SIR MCMC is also slower because each iteration requires repeated ODE solves, while the logistic MCMC avoids this burden.
Overall, the results show that moving from a closed-form model to an ODE-based model substantially increases computational expense.
...
This study investigated change-point detection in two nonlinear dynamical systems—a logistic growth model and an SIR epidemic model—using both frequentist and Bayesian approaches. Across both models, the results demonstrate that change-point models can accurately recover underlying parameters and identify structural changes when they are present.

A key finding is the strong agreement between frequentist and Bayesian inference in the logistic model. Parameter estimates showed negligible bias, and bootstrap confidence intervals closely matched Bayesian credible intervals. This consistency suggests that, in settings with relatively simple model structure and highly informative data, both paradigms provide reliable and robust inference.

In contrast, the SIR model reveals important differences. While parameter recovery remains accurate, uncertainty quantification differs more substantially between bootstrap and Bayesian approaches. The Bayesian framework better captures posterior dependence among parameters, leading to wider and more realistic uncertainty intervals in some cases. This highlights the importance of accounting for model-induced parameter correlations in mechanistic models. 

A central contribution of this work is the comparison of change-point identifiability across models. In the logistic model, the change-point posterior is concentrated but still exhibits small uncertainty, spreading across adjacent values. In the SIR model, however, the change-point is effectively identified without uncertainty, with the posterior collapsing to a point mass. This reflects the mechanistic nature of the SIR system, where changes in transmission rates propagate through the entire trajectory, making the timing of the change highly detectable.

From a computational perspective, model complexity plays a critical role. The logistic model is computationally efficient due to its closed-form structure, while the SIR model is substantially more expensive because it requires numerical ODE solutions at each likelihood evaluation. This leads to significantly higher runtime for both optimization and bootstrap procedures. Despite this, Bayesian MCMC remains competitive in the SIR setting, as it amortizes computational cost across iterations.

Overall, the results demonstrate that both inferential accuracy and computational cost depend strongly on model structure. Simpler models allow for fast and stable inference with agreement across methods, while more mechanistic models improve identifiability but introduce greater computational and inferential complexity.
